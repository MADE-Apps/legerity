...
/// <summary>
/// Defines a <see cref="AppiumElement"/> wrapper for the core UWP DatePicker control.
/// </summary>
public class DatePicker : WindowsElementWrapper
{
    ...
    public virtual void SetDate(DateTime date)
    {
        ...
        {
            popup.FindElement(WindowsByExtras.AutomationId("DayLoopingSelector")).FindElement(By.Name(date.ToString("%d"))).Click();
        }

        if (selectedMonth != date.ToString("MMMM"))
        {
            popup.FindElement(WindowsByExtras.AutomationId("MonthLoopingSelector")).FindElement(By.Name(date.ToString("MMMM"))).Click();
        }

        if (selectedYear != date.ToString("yyyy"))
        {
            popup.FindElement(WindowsByExtras.AutomationId("YearLoopingSelector")).FindElement(By.Name(date.ToString("yyyy"))).Click();
        }
    ...
    {
        AppiumElement button = this.FindElement(WindowsByExtras.AutomationId("FlyoutButton"));
