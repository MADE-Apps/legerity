...
/// <summary>
/// Defines a <see cref="AppiumElement"/> wrapper for the UWP MenuFlyoutSubItem control.
/// </summary>
public class MenuFlyoutSubItem : WindowsElementWrapper
{
    ...
    /// </param>
    public MenuFlyoutSubItem(AppiumElement element)
        : base(element)
    {
    }
    ...
    {
        return new MenuFlyoutSubItem(element as AppiumElement);
    }
    ...
    public virtual MenuFlyoutItem ClickChildOption(string name)
    {
        MenuFlyoutItem item = this.ChildMenuItems.FirstOrDefault(
            element => element.GetName()
                .Equals(name, StringComparison.CurrentCultureIgnoreCase)) ?? throw new NoSuchElementException($"Unable to find element using {name}");
        item.Click();
        return item;
    }
    ...
    public virtual MenuFlyoutItem ClickChildOptionByPartialName(string name)
    {
        MenuFlyoutItem item = this.ChildMenuItems.FirstOrDefault(
            element => element.GetName()
                .Contains(name, CultureInfo.CurrentCulture, CompareOptions.IgnoreCase)) ?? throw new NoSuchElementException($"Unable to find element using {name}");
        item.Click();
        return item;
    }
