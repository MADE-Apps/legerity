...
/// <summary>
/// Defines a <see cref="AppiumElement"/> wrapper for the core UWP GridView control.
/// </summary>
public class GridView : WindowsElementWrapper
{
    ...
    /// </param>
    public GridView(AppiumElement element)
        : base(element)
    {
    }
    ...
    /// </summary>
    public virtual ReadOnlyCollection<AppiumElement> Items =>
    ...
    {
        return new GridView(element as AppiumElement);
    }
    ...
    public virtual void ClickItem(string name)
    {
        ...
        AppiumElement item = this.Items.FirstOrDefault(element => element.VerifyNameOrAutomationIdEquals(name)) ?? throw new NoSuchElementException($"Unable to find item {name}.");
        item.Click();
    }
    ...
    public virtual void ClickItemByPartialName(string partialName)
    {
        ...
        AppiumElement item =
            this.Items.FirstOrDefault(element => element.VerifyNameOrAutomationIdContains(partialName)) ?? throw new NoSuchElementException($"Unable to find item {partialName}.");
        item.Click();
    }
    ...
    public virtual void ClickItemByIndex(int index)
    {
        ...
        AppiumElement item = this.Items[index] ?? throw new NoSuchElementException($"Unable to find item at index {index}.");
        item.Click();
    }
}